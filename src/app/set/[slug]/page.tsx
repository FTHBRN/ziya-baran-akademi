      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-3 border-b border-slate-200">
        <div className="space-y-1">
          <div className="flex items-center gap-2 text-xs font-semibold text-brand-600 uppercase tracking-wider">
            <Link href="/" className="hover:underline flex items-center gap-1 text-slate-500 hover:text-brand-600">
              <ArrowLeft className="w-3.5 h-3.5" />
              <span>Dersler</span>
            </Link>
            <span>/</span>
            <span>{setInfo.folders?.classes?.name || 'Genel'}</span>
            <span>/</span>
            <span>{setInfo.folders?.name || 'Set'}</span>
          </div>
        ...
        {/* Optional Konu Anlatımı Button (Only visible if studyNotes has content) */}
        {studyNotes && (
          <button
            type="button"
            onClick={() => setShowNotesModal(true)}
            className="self-start sm:self-auto inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs sm:text-sm font-semibold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 shadow-xs transition-all active:scale-95 shrink-0"
            title="Konu Anlatımı ve Ders Notlarını Oku"
          >
